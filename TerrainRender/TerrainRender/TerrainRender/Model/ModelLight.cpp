...
//}
double ModelLight::GetElevation(void) const
{
	return this->_elevation;
}

double ModelLight::GetAzimuth(void) const
{
...
double ModelLight::CalculateJulianDate(ModelTime time) {
...
void ModelLight::CalculateCelestialCoordinates(double n, double& alpha, double& delta)
{
...
double ModelLight::CalculateLocalHourAngle(double n, int hour, double alpha, double longitude)
{
...
void ModelLight::CalculateAzimuthAndElevation(double lat, double delta, double lha)
{
	lat = degToRad(lat);
	//https://en.wikipedia.org/wiki/Solar_zenith_angle
	//https://sci-hub.se/https://www.sciencedirect.com/science/article/abs/pii/S0038092X00001560
	//https://sci-hub.se/10.1016/0038-092x(88)90045-x
	_elevation = asin((sin(lat) * sin(delta)) + (cos(lat) * cos(delta) * cos(lha)));
	_azimuth = atan2(-sin(lha), tan(delta) * cos(lat) - sin(lat) * cos(lha));
	if (_azimuth < 0.0)
	{
		_azimuth += 2 * PI;
	}
...
void ModelLight::CalculateSunPosition(ModelTime time, double lat, double longitude)
{
	this->_currentTime = time;
	this->_currentLocation.lat = lat;
	this->_currentLocation.longitude = longitude;
